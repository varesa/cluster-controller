...
#[macro_export]
macro_rules! create_set_status_namespaced {
    ...
    ($resource_type:ident, $resource_status_type:ident) => {
        create_set_status_namespaced!($resource_type, $resource_status_type, set_status);
    };
}

#[macro_export]
macro_rules! create_set_status_cluster_scoped {
    ($resource_type:ident, $resource_status_type:ident, $fn_name:ident) => {
        #[instrument(skip(client))]
        pub async fn $fn_name(resource: &$resource_type, status: $resource_status_type, client: Client) -> Result<(), Error> {
            let api: Api<$resource_type> = Api::all(
                client.clone(),
            );
            let status_update = json!({
                "apiVersion": $resource_type::api_version(&()),
    ...
    ($resource_type:ident, $resource_status_type:ident) => {
        create_set_status_cluster_scoped!($resource_type, $resource_status_type, set_status);
    };
}
